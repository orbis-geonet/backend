...
function normalizeForRtDb(modifiedFilePath: string) {
...
export const modifyVideo = async ({
    ...
    try {
        ...
        const metadata: { [key: string]: any } = {
            ...
            contentEncoding: objectMetadata.contentEncoding,
            contentLanguage: objectMetadata.contentLanguage,
            contentType: supportedImageContentTypeMap.jpg,
            metadata: objectMetadata.metadata || {},
        };
        metadata.metadata.resizedImage = true;
        if (config.cacheControlHeader) {
...
const convertFfmpeg = (originalFile: string, modifiedFile: string, size: string): Promise<String> => {
    const s = size.split("x");
    if ( s.length != 2) {
        return new Promise<String>((r, e) => e(new Error(`Incorrect size: ${size}`)));
    }
    ...
    logger.log('[spawn] childProcess.pid: ', childProcess.pid);
    childProcess.stdout.on('data', function (data) {
        logger.log('[spawn] stdout: ', data.toString());
    });
    childProcess.stderr.on('data', function (data) {
        logger.log('[spawn] stderr: ', data.toString());
    });
